...
locals {
  cluster_instance_public_ips  = join(",", google_compute_instance.riak.*.network_interface.0.access_config.0.nat_ip )
  cluster_instance_private_ips = join(",", google_compute_instance.riak.*.network_interface.0.network_ip )
}
